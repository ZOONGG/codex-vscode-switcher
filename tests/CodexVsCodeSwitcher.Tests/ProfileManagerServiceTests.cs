...
public sealed class ProfileManagerServiceTests
{
    ...
    public void ListProfiles_AppliesDisplayMetadataAndOrder()
    {
        ...
        var store = new ProfileMetadataStore(temp.Paths.ProfilesMetadataFile, temp.ProtectedPaths);
        ...
        var service = new ProfileManagerService(
            temp.Paths,
            new ProfileDiscoveryService(temp.Paths.ProfilesDirectory, temp.ProtectedPaths),
            store,
            temp.ProtectedPaths);
    ...
    private static ProfileManagerService CreateService(TestLayout temp)
    {
        return new ProfileManagerService(
            temp.Paths,
            new ProfileDiscoveryService(temp.Paths.ProfilesDirectory, temp.ProtectedPaths),
            new ProfileMetadataStore(temp.Paths.ProfilesMetadataFile, temp.ProtectedPaths),
            temp.ProtectedPaths);
    }
}
