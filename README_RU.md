...
Интерфейс автоматического провайдера реализован, но автоматическое получение лимитов аккаунта Codex сейчас **недоступно**. Codex CLI `0.142.5` не предоставляет поддерживаемой machine-readable команды usage/limits, а официальная документация Codex не описывает локальный usage API, IPC-контракт или несекретный файл с такими данными. Поэтому главный переключатель недоступен, фоновые процессы проверки не запускаются, а оверлей не показывает выдуманные эмодзи или нулевые значения.

Когда появится поддерживаемый источник, оверлей останется минимальным и будет показывать только один эмодзи:
