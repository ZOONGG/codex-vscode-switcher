...
Экспериментальный автоматический провайдер использует официальную интерактивную команду Codex CLI `/status`. Для каждого сохранённого профиля он запускает скрытую Windows ConPTY-сессию с `CODEX_HOME`, указывающим на папку этого профиля, выполняет `codex --no-alt-screen`, отправляет `/status`, разбирает только известные строки лимитов и выходит через `/quit`. Перед включением **Экспериментальных автоматических индикаторов лимитов** используйте кнопку **Проверить поддержку** в настройках.

В кэш автоматических данных попадают только: название окна лимита, оставшийся процент, время сброса, время получения снимка, версия Codex CLI и безопасный источник `codex-cli-status`. Сырой вывод терминала, email аккаунта, session ID, модель, разрешения, директория, путь проекта и содержимое авторизации сразу отбрасываются и не пишутся в логи или фикстуры.
