...
### Политика резервных копий отката

Каждая новая копия отката содержит только `previous-auth.json`, прежние метаданные активного профиля и небольшой manifest с хешами и временем. Сессии, rollout-файлы, вложения, настройки, базы данных, логи, кэши и данные проектов никогда не копируются. Файл авторизации больше 10 МБ или копия переключения больше 25 МБ отклоняются до изменения состояния аккаунта.

При запуске и после переключения программа хранит не более пяти завершённых копий, удаляет копии старше семи дней, ограничивает их общий размер 100 МБ и удаляет только однозначно распознанные брошенные временные транзакции старше 24 часов. Активные транзакции не удаляются. В разделе **Настройки → Дополнительно** показан размер хранилища; там можно открыть папку, удалить завершённые копии или после явного подтверждения очистить старые каталоги `state-*`, сохранив две самые новые копии.
